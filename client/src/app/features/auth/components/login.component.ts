      ...
      >
        ...
        >
          <div class="p-6 space-y-4 md:space-y-6 sm:p-8 flex flex-col items-center">
            ...
            <form class="space-y-4 md:space-y-6" [formGroup]="loginForm" (ngSubmit)="handleSubmit()">
              <div>
                <label
                  for="email"
              ...
              <button
                ...
                <svg *ngIf="isLoading()" aria-hidden="true" role="status" class="inline w-4 h-4 me-3 text-gray-200 animate-spin dark:text-gray-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <span *ngIf="isLoading(); else signin">Loading...</span>
                  <ng-template #signin>
                    <span>Sign in</span>
                  </ng-template>
              </button>
  ...
  handleSubmit() {
    if(this.loginForm.valid) {
      ...
      this.#authService.signin({ email , password }).subscribe(response => {
        if(response.success) {
          const {_id, fullname, email }: { _id: string, fullname: string, email: string} = jwtDecode(response.data);

          this.#authService.authState.set({
            _id, fullname, email, jwt: response.data
          });
